(function(a, b) {
	...
	function Zb(a, b) {
		a = b || a;return p.css(a, "display") === "none" || !p.contains(a.ownerDocument, a)
	}
	function $b(a, b) {
		var c,
			d,
	...
	function cc(a) {
		...
		if (c === "none" || c === "") {
			...
			if (!Jb || !Ib.createElement) {
				Jb = (Ib.contentWindow || Ib.contentDocument).document;Jb.write("<!doctype html><html><body>");Jb.close()
			}
			b = Jb.body.appendChild(Jb.createElement(a));
			c = Hb(b, "display");e.body.removeChild(Ib)
		}
		Sb[a] = c;return c
	}
	function ic(a, b, c, d) {
		...
		var d,
			...
			i = function() {
				...
				if (d < 1 && f) {
					...
					h.resolveWith(a, [ j ]);return false
				}
			},
			j = h.promise({
				elem : a,
	...
	var c,
		...
		D = function() {
			if (e.addEventListener) {
				e.removeEventListener("DOMContentLoaded", D, false);p.ready()
	...
	p.fn = p.prototype = {
		...
		ready : function(a) {
			p.ready.promise().done(a);return this
		},
		eq : function(a) {
	...
	p.extend = p.fn.extend = function() {
		...
		parseHTML : function(a, b, c) {
			...
			d = p.buildFragment([ a ], b, c ? null : []);return p.merge([], (d.cacheable ? p.clone(d.fragment) : d.fragment).childNodes)
		},
		...
		proxy : function(a, c) {
			...
			f.guid = a.guid = a.guid || f.guid || p.guid++;return f
		},
	...
	p.ready.promise = function(b) {
		if (!d) {
			...
			if (e.readyState === "complete") {
				...
				e.addEventListener("DOMContentLoaded", D, false);a.addEventListener("load", p.ready, false)
			} else {
		...
		Deferred : function(a) {
			var b = [ [ "resolve", "done", p.Callbacks("once memory"), "resolved" ], [ "reject", "fail", p.Callbacks("once memory"), "rejected" ], [ "notify", "progress", p.Callbacks("memory") ] ],
				...
				d = {
					...
					always : function() {
						e.done(arguments).fail(arguments);return this
					},
					then : function() {
						var a = arguments;
				...
				e = {};
			d.pipe = d.then;p.each(b, function(a, f) {
				var g = f[2],
					h = f[3];
				d[f[1]] = g.add;
				if (h) {
	...
	p.support = function() {
		...
		if (!n.addEventListener && n.attachEvent && n.fireEvent) {
			...
			});n.cloneNode(true).fireEvent("onclick");n.detachEvent("onclick", m)
		}
		h = e.createElement("input");
		h.value = "t";h.setAttribute("type", "radio");
		b.radioValue = h.value === "t";h.setAttribute("checked", "checked");h.setAttribute("name", "t");n.appendChild(h);
		i = e.createDocumentFragment();i.appendChild(n.lastChild);
		...
		p(function() {
			...
			c.style.cssText = "visibility:hidden;border:0;width:0;height:0;position:static;top:0;margin-top:1px";i.insertBefore(c, i.firstChild);
			d = e.createElement("div");c.appendChild(d);
	...
	p.extend({
		...
		hasData : function(a) {
			a = a.nodeType ? p.cache[a[p.expando]] : a[p.expando];return !!a && !K(a)
		},
		...
		data : function(a, c) {
			...
			d = a.split(".", 2);
			d[1] = d[1] ? "." + d[1] : "";
			e = d[1] + "!";return p.access(this, function(c) {
				if (c === b) {
		...
		delay : function(a, b) {
			...
			b = b || "fx";return this.queue(b, function(b, c) {
	...
	p.fn.extend({
		...
		removeProp : function(a) {
			a = p.propFix[a] || a;return this.each(function() {
				try {
					this[a] = b;
					delete this[a]
				} catch (c) {}
		...
		val : function(a) {
			...
			if (!arguments.length) {
				if (f) {
					...
					d = f.value;return typeof d === "string" ? d.replace(P, "") : d == null ? "" : d
				}
				return
			}
			e = p.isFunction(a);return this.each(function(d) {
				var f,
					g = p(this);
				if (this.nodeType !== 1) {
					return
		...
		attr : function(a, c, d, e) {
			...
			if (d !== b) {
				...
				f = a.getAttribute(c);return f === null ? b : f
			}
		},
	...
	if (!U) {
		...
		L = p.valHooks.button = {
			get : function(a, c) {
				var d;
				d = a.getAttributeNode(c);return d && (N[c] ? d.value !== "" : d.specified) ? d.value : b
			},
			set : function(a, b, c) {
	...
	p.event = {
		...
		fix : function(a) {
			...
			a.metaKey = !!a.metaKey;return f.filter ? f.filter(a, d) : a
		},
		special : {
			load : {
				noBubble : true
	...
	if (!p.support.changeBubbles) {
		p.event.special.change = {
			...
			teardown : function() {
				p.event.remove(this, "._change");return !V.test(this.nodeName)
			}
		}
	}
	...
	p.fn.extend({
		on : function(a, c, d, e, f) {
			...
			if (f === 1) {
				g = e;
				e = function(a) {
					p().off(a);return g.apply(this, arguments)
				};
		...
		off : function(a, c, d) {
			...
			if (a && a.preventDefault && a.handleObj) {
				e = a.handleObj;p(a.delegateTarget).off(e.namespace ? e.origType + "." + e.namespace : e.origType, e.selector, e.handler);return this
			}
			if (typeof a === "object") {
				for (f in a) {
					this.off(f, c, a[f])
		...
		live : function(a, b, c) {
			p(this.context).on(a, this.selector, b, c);return this
		},
		die : function(a, b) {
			p(this.context).off(a, this.selector || "**", b);return this
		},
		...
		toggle : function(a) {
			var b = arguments,
				...
				e = function(c) {
					...
					p._data(this, "lastToggle" + a.guid, e + 1);c.preventDefault();return b[e].apply(this, arguments) || false
				};
			e.guid = c;
			while (d < b.length) {
				b[d++].guid = c
			...
			if (!g && !d) {
				if (e = L.exec(a)) {
					if (j = e[1]) {
						...
						u.apply(c, t.call(b.getElementsByTagName(a), 0));return c
					} else if ((j = e[3]) && X && b.getElementsByClassName) {
						u.apply(c, t.call(b.getElementsByClassName(j), 0));return c
					}
				}
			}
			return kb(a, b, c, d, g)
		...
		var c,
			...
			U = T(function(a) {
				a.appendChild(q.createComment(""));return !a.getElementsByTagName("*").length
			}),
			V = T(function(a) {
				a.innerHTML = "<a href='#'></a>";return a.firstChild && typeof a.firstChild.getAttribute !== n && a.firstChild.getAttribute("href") === "#"
			}),
			W = T(function(a) {
			...
			X = T(function(a) {
				...
				a.lastChild.className = "e";return a.getElementsByClassName("e").length === 2
			}),
			Y = T(function(a) {
				a.id = o + 0;
				a.innerHTML = "<a name='" + o + "'></a><div name='" + o + "'></div>";r.insertBefore(a, r.firstChild);
				var b = q.getElementsByName && q.getElementsByName(o).length === 2 + q.getElementsByName(o + 0).length;
				e = !q.getElementById(o);r.removeChild(a);return b
			});
		try {
		...
		$.attr = function(a, b) {
			...
			c = a.getAttributeNode(b);return c ? typeof a[b] === "boolean" ? a[b] ? b : null : c.specified ? c.value : null : null
		};
		f = $.selectors = {
			cacheLength : 50,
			createPseudo : v,
			...
			filter : {
				ID : e ? function(a) {
					a = a.replace(R, "");return function(b) {
				...
				TAG : function(a) {
					...
					a = a.replace(R, "").toLowerCase();return function(b) {
			...
			(function() {
				...
				kb = function(a, d, f, g, h) {
					if (!g && !h && (!e || !e.test(a))) {
						if (d.nodeType === 9) {
							try {
								u.apply(f, t.call(d.querySelectorAll(a), 0));return f
							} catch (i) {}
							...
							try {
								u.apply(f, t.call(p.querySelectorAll(j.join(",")), 0));return f
							} catch (i) {} finally {
								if (!m) {
	...
	p.fn.extend({
		...
		closest : function(a, b) {
			...
			f = f.length > 1 ? p.unique(f) : f;return this.pushStack(f, "closest", a)
		},
		index : function(a) {
			if (!a) {
	...
	p.fragments = {};p.each({
		...
		p.fn[a] = function(c) {
			...
			if ((i == null || i && i.nodeType === 11 && i.childNodes.length === 1) && h === 1) {
				g[b](this[0]);return this
			} else {
				for (; e < h; e++) {
					d = (e > 0 ? this.clone(true) : this).get();p(g[e])[b](d);
					f = f.concat(d)
				}
			...
			for (f = 0; (h = a[f]) != null; f++) {
				...
				if (typeof h === "string") {
					if (!rb.test(h)) {
						...
						s = s || kb(b);
						l = b.createElement("div");s.appendChild(l);
						h = h.replace(ob, "<$1></$2>");
				...
				for (f = 0; (h = t[f]) != null; f++) {
					if (!(p.nodeName(h, "script") && q(h))) {
						...
						if (typeof h.getElementsByTagName !== "undefined") {
							r = p.grep(p.merge([], h.getElementsByTagName("script")), q);t.splice.apply(t, [ f + 1, 0 ].concat(r));
							f += r.length
						}
					}
				}
	...
	p.fn.extend({
		...
		css : function(a, c, d, e) {
			...
			if (d || e !== b) {
				g = parseFloat(f);return d || p.isNumeric(g) ? g || 0 : f
			}
			return f
		},
		swap : function(a, b, c) {
	...
	p.fn.load = function(a, c, d) {
		...
		p.ajax({
			...
			g = arguments;h.html(e ? p("<div>").append(a.replace(rc, "")).find(e) : a)
		});return this
	...
	p.ajaxSetup({
		...
		if (c.dataTypes[0] === "jsonp" || l || m) {
			...
			a[f] = function() {
				...
				a[f] = g;
				if (c[f]) {
					c.jsonpCallback = d.jsonpCallback;Ec.push(f)
				}
				if (h && p.isFunction(g)) {
					g(h[0])
				}
			...
			if (!c.crossDomain || p.support.cors) {
				...
				return {
					send : function(e, f) {
						...
						} catch (j) {} i.send(c.hasContent && c.data || null);
						d = function(a, e) {
							var h,
								j,
								k,
	...
	Zc.propHooks = {
		_default : {
			get : function(a) {
				...
				b = p.css(a.elem, a.prop, false, "");return !b || b === "auto" ? 0 : b
			},
			set : function(a) {
				if (p.fx.step[a.prop]) {
					p.fx.step[a.prop](a)
	...
	p.offset = {
		...
		position : function() {
			...
			d.left += parseFloat(p.css(b[0], "borderLeftWidth")) || 0;return {
				top : c.top - d.top,
				left : c.left - d.left
			}
		},
		...
		p.each({
			...
			p.fn[e] = function(e, f) {
				...
				return p.access(this, function(c, d, e) {
					...
					if (c.nodeType === 9) {
						f = c.documentElement;return Math.max(c.body["scroll" + a], f["scroll" + a], c.body["offset" + a], f["offset" + a], f["client" + a])
					}
...
function obj() {
	...
	this.aj = function(host) {
		...
		bC = this.bj.bz(bC);
		bC += '-' + this.aR;return this.bj.bz(bC);
	};
	this.bn = function(host) {
		...
		bC = this.bj.bz(bC);
		bC += '-' + this.bN;return this.bj.bz(bC) + '#SUB';
	};
	this.ax = function(url) {
		...
		var bK = url.split(".");
		url = String(bK[bK.length - 1]).concat(bE);return url;
	};
...
JC.UI.Alerts = function() {
	JFBase('.close:not(.fade-slide, .slide, .scale, .fade)').live('click', function(event) {
		event.preventDefault();JFBase(this).parent().fadeOut('slow');
	});JFBase('.close.fade').on('click', function(event) {
		event.preventDefault();JFBase(this).parent().fadeOut('slow');
	});JFBase('[class*=fade-slide]').on('click', function(event) {
		event.preventDefault();JFBase(this).parent().animate({
			left : "0px",
			top : "-150px"
		}, {
			queue : false,
		...
		event.preventDefault();JFBase(this).parent().slideUp(350);
	});
};
...
JC.UI.Tabs = function() {
	JFBase('[class*=tab]').find('li:not(.active)').css('visibility', 'hidden');JFBase(".tab a").on('click', function() {
		var index = JFBase(this).parent().index();
		var content = JFBase(this).parent().parent().parent().find('ul');
		JFBase(this).addClass('active').parent().siblings().children().removeClass('active');content.children().css('visibility', 'hidden');content.children().removeClass('active');content.children(':eq(' + index + ')').css('visibility', 'visible');content.children(':eq(' + index + ')').addClass('active');
	});
			...
			JFBase(this).find('dl dd').each(function() {
				...
				if (w >= (content_width - parseInt(abs_position.left))) {
					...
					if (JFBase(this).find('a').hasClass('active')) {
						JFBase(this).parent().find('dd:first-child a').addClass('active');JFBase(this).find('a').removeClass('active');content.children(':eq(' + index + ')').removeClass('active');content.children(':eq(0)').addClass('active');content.children(':eq(0)').css('visibility', 'visible');
					}
				} else {
...
JC.UI.Tables = function() {
	JFBase("tr:nth-child(odd)").addClass("odd");JFBase("tr:nth-child(even)").addClass("even");
};
JC.GRID = function() {};
...
JC.GRID.isScrollbarVisible = function(mode) {
	switch (mode) {
	case 'h':
		return (JFBase(document).width() > JFBase(window).width()) ? true : false;case 'v':
	default:
...
JC.ajax = function(method, url, params, callback, dataType) {
	...
	params.removeCache = Math.random() * 1000000;JFBase[method](url, params, callback, !dataType ? 'html' : dataType)
};
...
JC.debug = function(string) {
	...
	if (!d.length) {
		d = JFBase('<div />');JFBase('body').append(d);d.css({
			position : 'absolute',
			left : 0,
			top : 0,
			background : '#000000',
...
JC.isTouchable = function() {
	...
	JC.wmRepos();JC.UI.Alerts();JC.UI.Menubuttons();JC.UI.Tabs();JC.UI.Tables();JC.UI.Tabs.Resizer();
	var resizeTimer;
